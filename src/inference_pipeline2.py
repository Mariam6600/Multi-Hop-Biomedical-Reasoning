"""
src/inference_pipeline2.py
===========================
Biomedical Multi-Hop QA Project — Baseline 2

The main orchestrator that runs the full inference pipeline with BM25 retrieval.

What this module does:
  1. Loads normalized dataset from data/medhop.json
  2. For each question:
     - Retrieves top-K supporting texts using BM25
     - Builds prompt WITH context
     - Sends to BioMistral via llm_runner
     - Extracts clean Drug ID from response
  3. Saves predictions incrementally
  4. Calculates Exact Match (EM) score
  5. Saves final predictions + logs to outputs/

Output files:
  outputs/baseline2_predictions.json  ← predictions + ground truth
  outputs/baseline2_logs.json         ← timing + stats + EM score
...
from config.settings import (
    ...
    MAX_QUESTIONS,
    LOG_EVERY_N,
    OLLAMA_MODEL_NAME,  # <--- تم التصحيح هنا
    BM25_TOP_K,
    BM25_METHOD,
)
...
# استيراد دالة التحقق من وجود الإجابة
from src.retriever_semantic import check_answer_in_retrieved
...
def load_existing_predictions() -> dict:
    ...
    """
    if not os.path.exists(PREDICTIONS_FILE_B2):
        return {}
...
def save_logs(logs: dict):
    ...
    os.makedirs(OUTPUTS_DIR, exist_ok=True)
    with open(LOGS_FILE_B2, "w", encoding="utf-8") as f:
...
def run_pipeline():
    """
    Full Baseline 2 inference pipeline.
    Runs all questions through BM25 + BioMistral and saves results.
    """

    print("\n" + "=" * 60)
    print("  Baseline 2 — BM25 RAG Pipeline")
    print("=" * 60)
    print(f"  Model   : {OLLAMA_MODEL_NAME}") # <--- تم التصحيح هنا
    print(f"  Method  : BM25 ({BM25_METHOD})")
    print(f"  Top-K   : {BM25_TOP_K}")
    print(f"  Dataset : {MEDHOP_FILE}")
    print(f"  Output  : {PREDICTIONS_FILE_B2}")
    print()
    ...
    print("--- Step 4: Running BM25 Retrieval + Inference ---")
    ...
    logs = {
        "run_info": {
            "baseline": "Baseline 2 - BM25 RAG",
            "model": OLLAMA_MODEL_NAME, # <--- تم التصحيح هنا
    ...
    print()
    print("=" * 60)
    print("  BASELINE 2 — FINAL RESULTS")
    print("=" * 60)
    print(f"  Method          : BM25 RAG (Top-{BM25_TOP_K})")
    print()
    ...
    print(f"  📊  Recall@{BM25_TOP_K} Score        : {avg_recall:.2f}%")
    print()
    ...
    print()
    print(f"  Predictions saved : {PREDICTIONS_FILE_B2}")
    print(f"  Logs saved        : {LOGS_FILE_B2}")
    print("=" * 60)
    print()
