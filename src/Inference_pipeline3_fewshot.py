"""
src/inference_pipeline3_fewshot.py
====================================
Biomedical Multi-Hop QA Project — Baseline 3 + Few-Shot Experiment

Same as inference_pipeline3.py but uses Few-Shot + MedCPT context prompt.
Combines the best of:
  - Baseline 1 EXP3 (Few-Shot pattern → EM 15.79%)
  - Baseline 3 (MedCPT semantic retrieval → EM 16.37%)

Output files are separate so original Baseline 3 results are preserved.
...
from config.settings import (
    MEDHOP_FILE,
    PREDICTIONS_FILE_B3_FEWSHOT as PREDICTIONS_FILE_B3,
    LOGS_FILE_B3_FEWSHOT as LOGS_FILE_B3,
    OUTPUTS_DIR,
    MAX_QUESTIONS,
    LOG_EVERY_N,
    OLLAMA_MODEL_NAME,
...
def run_pipeline():
    """Full Baseline 3 + Few-Shot inference pipeline."""

    print("\n" + "=" * 60)
    print("  Baseline 3 — FewShot + MedCPT Semantic RAG Pipeline")
    print("=" * 60)
    print(f"  Model    : {OLLAMA_MODEL_NAME}")
    print(f"  Retrieval: MedCPT (Semantic)")
    print(f"  Prompt   : Few-Shot + Context")
    ...
    print("--- Step 4: Running MedCPT Retrieval + FewShot Inference ---")
    ...
    for i, record in enumerate(data):
        ...
        # ── Build Few-Shot + Context prompt ──
        prompt = build_prompt_fewshot_with_context(record, retrieved)
    ...
    logs = {
        "run_info": {
            "baseline": "Baseline 3 - FewShot + MedCPT Semantic RAG",
    ...
    print()
    print("=" * 60)
    print("  BASELINE 3 FEWSHOT — FINAL RESULTS")
    print("=" * 60)
    print(f"  Method          : FewShot + MedCPT Semantic RAG (Top-{MEDCPT_TOP_K})")
    print()
    ...
    print(f"  📊  Recall@{MEDCPT_TOP_K} Score        : {avg_recall:.2f}%")
    print()
